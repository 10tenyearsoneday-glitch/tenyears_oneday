...
(() => {
  ...
  // ===== GAS POST (CORS 回避版：hidden iframe + postMessage) =====
  // GAS 端需支援：doPost 回傳 HtmlService，並 postMessage 回父頁（見下方提供的 GAS 修正版）
  function gasPost(path, payload, { admin = false, timeoutMs = 15000 } = {}) {
    return new Promise((resolve, reject) => {
      const rid = "ten_req_" + Date.now() + "_" + Math.random().toString(16).slice(2);
      ...
      const url =
        ...
        (admin ? `&key=${encodeURIComponent(ADMIN_KEY)}` : "") +
        `&postMessage=1&rid=${encodeURIComponent(rid)}`;
      form.action = url;
  ...
  // 讓其他頁也能用（member.html 也可直接呼叫）
  window.TEN.gasPost = gasPost;
  ...
  async function submitOrderFromModal() {
    ...
    try {
      ...
      const out = await gasPost("order_create", payload, { admin: true }).catch(() => null);
